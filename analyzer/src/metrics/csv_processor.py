...
class CSVBenchmarkProcessor:
    ...
    def __init__(
        self,
        static_metrics: List[StaticMetric] = None,
    ...
    def process_csv(self, input_file: str, output_file: str) -> pd.DataFrame:
        ...
        for idx, row in df.iterrows():
            print(
                f"Processing benchmark {idx + 1}/{len(df)}: {row.get('name', 'unknown')}"
            )
...
def run_csv_processor_example():
    ...
    # Create metrics
    static_metrics = [NumSamplesMetric(), AvgTextLengthMetric(), NumClassesMetric()]

    dynamic_metrics = [CitationCountMetric(), DownloadCountMetric()]
