...
- **`run_metrics.py`** - Main runner script that:
  - Initializes all 10 datasets with their metadata
  - Loads metadata from `data/helm_dataset_metadata.json`
  - Adds datasets to the HELM leaderboard
  - Runs all static metrics on each dataset
  - Prints results and rankings
...
This will:
1. Initialize all 10 HELM datasets
2. Load metadata from `data/helm_dataset_metadata.json`
3. Process each dataset and extract metadata
4. Run all static metrics on each dataset
5. Print metric results and dataset rankings

## Metadata Source

Dataset metadata is extracted from:
- **Source**: `data/helm_dataset_metadata.json`
- **Generated by**: `data/extract_helm_dataset_metadata.py`
